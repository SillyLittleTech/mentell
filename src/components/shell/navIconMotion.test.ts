...
describe('navMotionPlan — issue #206 animation skeleton', () => {
  it('envelope: letter slides down, envelope slides up', () => {
    ...
    expect(plan.letter?.animate).toMatchObject({ y: 8, opacity: 0 })
    expect(plan.primary?.initial).toMatchObject({ y: -10, opacity: 0 })
  ...
  it('projector: closed book expands then swaps to stories icon mid animation', () => {
    ...
    expect(plan.book?.initial).toMatchObject({ scale: 0.78 })
    expect(plan.book?.animate).toMatchObject({ scale: 1.08, opacity: 0 })
  ...
  it('notepad: paper rises from below', () => {
    ...
    expect(plan.single?.initial).toMatchObject({ y: 12 })
  ...
  it('shoppe: slides in from the left with italic slant then settles', () => {
    ...
    expect(plan.single?.initial).toMatchObject({ x: -14, skewX: -12, scale: 0.86 })
  ...
  it('settings: cog rotates into place', () => {
    ...
    expect(plan.single?.initial).toMatchObject({ rotate: -120 })
  ...
  it('character: quick horizontal shake keyframes', () => {
    ...
    expect(plan.shake?.x).toEqual([0, -3, 3, -2, 2, 0])
  })
  ...
  it('keeps animations quick but visible (under half a second each)', () => {
    for (const kind of [
      ...
      expect(navMotionDurationSec(kind)).toBeGreaterThan(0.2)
      expect(navMotionDurationSec(kind)).toBeLessThan(0.35)
    }
  })
})
