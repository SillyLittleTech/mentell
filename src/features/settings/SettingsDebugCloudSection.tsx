  ...
  return (
    <section className="paper rounded-3xl p-6">
      ...
      <p className="ink-muted mt-2 text-sm">
        Signed in as{" "}
        <span className="font-medium text-[var(--paper-ink)]">{label}</span>.
        Standard sign-in is disabled in debug builds; data syncs to a sandbox
        account, not your production Google user.
      </p>
      {isFirebaseSyncEnabled() && auth.syncEnabled ? (
        <p className="ink-muted mt-2 text-xs">
          Cloud sync is on for this debug session.
        </p>
      ) : null}
      {auth.syncError ? (
