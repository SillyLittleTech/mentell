          ...
          <label className="mt-4 flex items-center justify-between gap-3 text-sm">
            <span>
              ...
              <div className="ink-muted text-xs">
                Minimizes animations; also respects OS preference.
              </div>
            </span>
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                updateSettingsAndMarkDirty({ reducedMotion: e.target.checked })
              }
            />
          </label>
        </section>
          ...
          <div className="mt-4 grid gap-4">
            <label className="flex items-center justify-between gap-3 text-sm">
              <span>
                ...
                <div className="ink-muted text-xs">
                  Hides weekly AI on the Week tab (cloud synchronization still
                  required).
                </div>
              </span>
              <input
                ...
                onChange={(e) =>
                  updateSettingsAndMarkDirty({ disableAi: e.target.checked })
                }
              />
            </label>
            <label className="flex items-center justify-between gap-3 text-sm">
              <span>
                ...
                <div className="ink-muted text-xs">
                  Hides score/streak and disables Shoppe purchases.
                </div>
              </span>
              <input
                type="checkbox"
                ...
                onChange={(e) =>
                  updateSettingsAndMarkDirty({
                    disablePoints: e.target.checked,
                  })
                }
              />
            </label>
            <label className="flex items-center justify-between gap-3 text-sm">
              <span>
                ...
                <div className="ink-muted text-xs">
                  Stops permission prompts, in-app alerts, web push, and the
                  desktop weekly reminder.
                </div>
              </span>
              <input
                type="checkbox"
            ...
            <div className="grid gap-3 rounded-2xl border border-[var(--paper-border)] p-4">
              ...
              <p className="ink-muted text-xs">
                Weekly packages appear after this day and time, once that
                journal week is complete (Monday–Sunday). On the web, background
                push uses your timezone below — Chrome and Firefox on desktop
                are the most reliable; Safari (Mac and iOS Home Screen) is
                stricter and may hold alerts until you open the app. The desktop
                app schedules a native weekly reminder that still fires after
                you quit Mentell.
              </p>
              <label className="grid gap-1 text-sm">
                <span className="ink-muted text-xs font-medium">
                  Delivery day
                </span>
                <select
                  ...
                  onChange={(e) =>
                    updateSettingsAndMarkDirty({
                      deliveryWeekday: Number(e.target.value),
                    })
                  }
                >
              ...
              <label className="grid gap-1 text-sm">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSettingsAndMarkDirty({
                      deliveryTimeLocal: e.target.value,
                    })
                  }
                />
              </label>
              <label className="grid gap-1 text-sm">
                ...
                <input
                  ...
                  onFocus={() =>
                    updateSettingsAndMarkDirty({ timezone: browserTimezone() })
                  }
                />
