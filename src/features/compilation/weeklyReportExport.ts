...
export async function fetchEntriesForRange(
  ...
  const startKey = toDateKey(
    startOfWeek(subWeeks(anchor, 4), { weekStartsOn: 1 }),
  );
  const entriesNorm = await getDb()
    .entries.where("dateKey")
...
function buildSummaryCard(
  counts: ReturnType<typeof countSentiments>,
  maxBar: number,
...
function buildByWeekCard(
  byWeek: Map<string, EntryRow[]>,
  range: RawReportRange,
) {
  if (range === "week" || byWeek.size <= 1) return "";
  ...
  return `<div class="card"><h2>By week</h2>${weekSections}</div>`;
}

function buildEntriesCard(sortedEntries: EntryRow[]) {
...
<body>
${buildReportHeader(rangeLabel)}
${buildSummaryCard(counts, maxBar, globalName)}
${buildByWeekCard(byWeek, input.range)}
${buildEntriesCard(sorted)}
