...
import { normalizeEndpointUrl, normalizeEnvToken } from '../compilation/weeklyAiSummary'
...
async function sendDebugEmailTest(template: 'daily' | 'package', variables: Record<string, string>) {
  const apiBase = getWorkerApiBase()
  if (!apiBase) return `${template} email test: VITE_PUSH_API_BASE is not set`
  const token = normalizeEnvToken(import.meta.env.VITE_WEEKLY_AI_TOKEN)
  if (!token) return `${template} email test: VITE_WEEKLY_AI_TOKEN is not set`
  const email = loadAppSettings().notificationEmail.trim()
  if (!email) return `${template} email test: save a notification email in Settings first`

  try {
    const res = await fetch(`${apiBase}/email/test`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ email, template, variables }),
    })
    const text = await res.text().catch(() => '')
    let detail = text
    if (text) {
      try {
        const parsed = JSON.parse(text) as { error?: string; id?: string }
        detail = parsed.error || (parsed.id ? `id ${parsed.id}` : text)
      } catch {
        /* keep raw */
      }
    }
    if (res.ok) return `${template} email test: Sent${detail ? ` (${detail})` : ''}`
    return `${template} email test: Failed (${res.status} ${res.statusText || 'No Content'})${detail ? ` — ${detail}` : ''}`
  } catch (err) {
    return `${template} email test: ${err instanceof Error ? err.message : 'Network error'}`
  }
}
          ...
          >
            ...
            <button
              ...
              onClick={async () => {
                setBusy(true)
                try {
                  alert(await sendDebugEmailTest('daily', {
                    global_name: getEffectiveGlobalName(),
            ...
            >
              Test Daily Email
              <div className="ink-muted text-xs">Sends daily reminder to your saved email.</div>
            </button>
            <button
              type="button"
              disabled={busy}
              ...
              onClick={async () => {
                setBusy(true)
                try {
                  alert(await sendDebugEmailTest('package', {
                    global_name: getEffectiveGlobalName(),
                    date: new Date().toISOString().split('T')[0],
                    ent_count: '5',
                    ent_rank: '⬆',
                    ent_sum: 'You had a good week.',
                  }))
                } finally {
                  setBusy(false)
                }
            ...
            >
              Test Package Email
              <div className="ink-muted text-xs">Sends package drop email to your saved email.</div>
            </button>
            <button
              type="button"
              disabled={busy}
              ...
              onClick={async () => {
                setBusy(true)
                try {
                  saveAppSettings({ emailVerified: false })
                  const apiBase = getWorkerApiBase()
                  if (!apiBase) {
                    alert('Unverified locally. VITE_PUSH_API_BASE is not set, so the worker record was not cleared.')
                    return
                  }
                  const res = await fetch(`${apiBase}/email/unverify`, {
                    method: 'POST',
                    headers: {
                      'Content-Type': 'application/json',
                      ...(await getWorkerAuthHeaders()),
                    },
                    body: JSON.stringify({ clientId: getOrCreatePushClientId() }),
                  })
                  const text = await res.text().catch(() => '')
                  let detail = text
                  if (text) {
                    try {
                      const parsed = JSON.parse(text) as { error?: string; existed?: boolean }
                      detail = parsed.error || (parsed.existed === false ? 'no worker record' : 'worker record cleared')
                    } catch {
                      /* keep raw */
                    }
                  }
                  if (!res.ok) {
                    alert(`Unverify failed (${res.status}): ${detail}`)
                    return
                  }
                  alert(`Notification email unmarked as verified (${detail}). Save & Subscribe in Settings to send a new link.`)
                } finally {
                  setBusy(false)
                }
              }}
            >
              Unverify notification email
              <div className="ink-muted text-xs">Clears local + worker verified state so you can retest the link.</div>
            </button>
              </div>
            </div>
          </div>
