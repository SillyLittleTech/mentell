...
import {
  ProgressLight,
  type ProgressState,
} from "../../components/ProgressLight";
import {
  SentimentPills,
  type SentimentValue,
} from "../../components/SentimentPills";
...
export function LetterComposer({
  ...
  const [draftInputs, setDraftInputs] = useState<DraftInputState[]>([
    createDraftInput(),
  ]);
  const [isBulkMode, setIsBulkMode] = useState(false);

  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitState, setSubmitState] = useState<"idle" | "done" | "error">(
    "idle",
  );
  const [draftRisk, setDraftRisk] = useState<ReturnType<
    typeof assessDraftRisk
  > | null>(null);
  const [submittedRisk, setSubmittedRisk] = useState<RiskAssessment | null>(
    null,
  );
  ...
  useEffect(() => {
      ...
      setDraftInputs([
        {
          ...
          extrasOpen: Boolean(
            detail.behavioursNoted || detail.reoccurringTheme,
          ),
          timeframe: "just now",
        },
      ]);
    ...
    return () =>
      window.removeEventListener("mentell:debug-ai-test-fill", fillDebugAiTest);
  }, []);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      for (const draft of isBulkMode ? draftInputs : [draftInputs[0]]) {
        ...
        const hasMessage =
          finalRisk.responseKind !== "none" &&
          !!finalRisk.supportiveMessage?.trim();
        const prevIsCrisis = highestRisk?.responseKind === "crisis";
        const prevHasMessage =
          highestRisk?.responseKind !== "none" &&
          !!highestRisk?.supportiveMessage?.trim();
        ...
        if (!highestRisk) {
          ...
          !prevIsCrisis &&
          !prevHasMessage &&
          finalRisk.riskScore > highestRisk.riskScore
        ) {
          highestRisk = finalRisk;
        }
        ...
        finalDrafts.push({
          ...
          emotion: draft.emotion,
          emotionNote:
            draft.emotion === "other" ? draft.emotionNote.trim() : "",
    ...
    >
      ...
      <section className="paper rounded-3xl p-6">
        <header className="flex flex-wrap items-start justify-between gap-4">
          ...
          <SentimentPills
            value={draftInputs[0].sentiment}
            onChange={(val) =>
              disabled ? {} : updateDraft(draftInputs[0].id, { sentiment: val })
            }
          />
        </header>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateDraft(draftInputs[0].id, {
                      emotion: e.target.value as EntryEmotion,
                    })
                  }
                >
        ...
        {isBulkMode && draftInputs.length > 1 ? (
          <div className="mt-8 space-y-6">
            {draftInputs.slice(1).map((draft, index) => (
              <div
                key={draft.id}
                className="pt-6 border-t border-[var(--paper-border)] grid gap-5 relative"
              >
                <div className="absolute top-6 right-0">
                  <SentimentPills
                    value={draft.sentiment}
                    onChange={(val) =>
                      disabled ? {} : updateDraft(draft.id, { sentiment: val })
                    }
                  />
                </div>
...
function DraftRiskNotice({
  risk,
}: {
  risk: ReturnType<typeof assessDraftRisk>;
}) {
  return (
    <div className="space-y-1">
      ...
      <div>
        When you submit, Mentell can show support resources or a gentler note
        for what you wrote.
      </div>
      {isDebugMode() ? (
...
function RiskNotice({ risk }: { risk: RiskAssessment }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {risk.supportiveMessage?.trim() ? (
        <div>{risk.supportiveMessage}</div>
      ) : null}
      {isDebugMode() ? <RiskSignalLine risk={risk} /> : null}
      {isDebugMode() && risk.reasons.length ? (
        <div>Signals: {risk.reasons.join(", ")}</div>
      ) : null}
...
function RiskSignalLine({ risk }: { risk: RiskAssessment }) {
  const guard =
    risk.guardSafe === undefined
      ? ""
      : ` · guard ${risk.guardSafe ? "safe" : "unsafe"}`;
  return (
    <div className="font-mono text-[11px] uppercase opacity-70">
      Literal {risk.literalSentimentLabel}{" "}
      {risk.literalSentimentConfidence.toFixed(2)} (
      {risk.literalSentimentScore.toFixed(2)}) · {risk.sentimentModelSource} ·{" "}
      {risk.source}
      {guard}
    </div>
  );
...
function EntryExtrasFields({
  ...
  draft: Pick<
    DraftInputState,
    "behavioursNoted" | "reoccurringTheme" | "extrasOpen"
  >;
  disabled?: boolean;
  onChange: (updates: Partial<DraftInputState>) => void;
}) {
  const expanded = draft.extrasOpen;
  const behavioursOver = isOverLimit(
    draft.behavioursNoted,
    ENTRY_BEHAVIOURS_NOTED_MAX,
  );
  const themeOver = isOverLimit(
    draft.reoccurringTheme,
    ENTRY_REOCCURRING_THEME_MAX,
  );
  return (
    <div className="grid gap-3">
      <button
        ...
        aria-label={
          expanded ? "Hide extra entry details" : "Show extra entry details"
        }
