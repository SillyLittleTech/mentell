...
export function ScoreBurst({
  ...
  const [displayTotal, setDisplayTotal] = useState(
    reduced ? totalAfter : totalBefore,
  );
  const [displayDelta, setDisplayDelta] = useState(
    reduced ? Math.abs(delta) : 0,
  );
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        transition={{
          duration: motionDuration(0.4) || 0.01,
          ease: [0.2, 0.8, 0.2, 1],
        }}
      >
        <motion.div
        ...
        >
          {hint ? (
            <div className="ink-muted mb-3 max-w-xs text-sm">{hint}</div>
          ) : null}
