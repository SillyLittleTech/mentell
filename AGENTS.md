...
- Visible-tab poll in [`App.tsx`](src/App.tsx) every 60s runs delivery while the tab is focused. Background OS notifications (tab/PWA closed) come from the Worker push cron waking [`sw.ts`](src/pwa/sw.ts), not from this timer. Chrome/Firefox desktop Web Push is generally reliable; Safari (macOS and iOS Home Screen) is stricter and may hold pushes until the PWA is opened. The Tauri desktop app skips the service worker and uses native macOS notifications plus a repeating weekly `UserNotifications` schedule (survives quit). Windows/Linux Tauri only show immediate in-app banners (plugin `show()`); they do not schedule after quit.
...
- **Email notifications (optional, Resend):** Settings → Email Notifications. Same worker (`/email/subscribe`, `/email/verify`, `/email/test`). Requires `RESEND_API_KEY` + published template ids (`RESEND_TEMPLATE_VERIFY` / `_DAILY` / `_PACKAGE`). Client auth is Firebase ID token or `VITE_WEEKLY_AI_TOKEN` (must match `WEEKLY_SUMMARY_TOKEN`).
